"""
ZINC-12k Benchmark: HCN (α1/α2) + HPM + Cycle-Size Virtual Edge Encoding
...
Builds on the best architecture (HCN+HPM = 0.0772 MAE) by adding cycle-size
labels to virtual edges, enabling the model to distinguish 5-rings from 6-rings.
...
def prepare_zinc():
    cache_path = 'data/ZINC_FC_CYCLES_v4_SIZED.pt'

    if os.path.exists(cache_path):
        print("  [+] Loading cached ZINC subsets (FC-Cycles + Sized) from disk...")
        return torch.load(cache_path, map_location='cpu', weights_only=False)

    print("  [!] No cache found. Processing ZINC...")
...
class ZINC_Model(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.is_hcn:
            # Embed virtual edge cycle sizes
            v_edge_emb = self.virtual_edge_emb(data.v_edge_attr) if data.v_edge_attr.numel() > 0 else None
...
if __name__ == '__main__':
    ...
    print(f"\n{'=' * 85}")
    print(f" ZINC-12k (HCN + HPM + Cycle-Size Encoding) | LOWER IS BETTER")
    ...
    # Final Table
    print(f"\n{'=' * 115}")
    print(f" ZINC-12k ABLATION: Original HCN + HPM (LOWER IS BETTER)")
    print(f"{'=' * 115}")
